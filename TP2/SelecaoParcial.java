...
public class SelecaoParcial {
    ...
    public static void selecaoParcial (List<Personagem> subArray, int n) {
        for (int i = 0; i < 10; i++) {
            int menor = i;
            for (int j = (i + 1); j < n; j++) {
                if (subArray.get(menor).getName().compareTo(subArray.get(j).getName()) > 0) {
                    menor = j;
                }
            }
            Personagem temp = subArray.get(i);
            subArray.set(i, subArray.get(menor));
            subArray.set(menor, temp);
        }
    }

    public static void log(int comp, int mov, double fim) throws Exception {
        RandomAccessFile escrever = new RandomAccessFile("matrícula_countingsort.txt", "rw");
    ...
    public static void main(String[] args) throws Exception {
        ...
        selecaoParcial(subArray, subArray.size());
