...
public class Insercao {
    ...
    public static void ler(Personagem[] array) throws Exception {
        RandomAccessFile arq = new RandomAccessFile("\\tmp\\characters.csv",
                "r");
    ...
    public static void insercao(List<Personagem> subarray, int n) {
        for (int i = 1; i < n; i++) {
            Personagem tmp = subarray.get(i);
            int j = i - 1;
    
            while ((j >= 0) && (subarray.get(j).getDateOfBirth().getTime() > tmp.getDateOfBirth().getTime() ||
                                (subarray.get(j).getDateOfBirth().getTime() == tmp.getDateOfBirth().getTime() &&
                                 subarray.get(j).getName().compareTo(tmp.getName()) > 0))) {
                subarray.set(j + 1, subarray.get(j));
                j--;
                comp++;
                mov++;
            }
            subarray.set(j + 1, tmp);
            mov++;
        }
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        insercao(subArray, subArray.size());
