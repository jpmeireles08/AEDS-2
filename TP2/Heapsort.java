...
public class Heapsort {
    ...
    public static void log(int comp, int mov, double fim) throws Exception {
        RandomAccessFile escrever = new RandomAccessFile("matrícula_heapsort.txt", "rw");
    ...
    public static void sort(List<Personagem> subarray, int n) {
        Personagem[] tmp = new Personagem[n];
    
        for (int i = 0; i < n; i++) {
            tmp[i] = subarray.get(i);
            mov++;
        }
    
        for (int tamHeap = n / 2 - 1; tamHeap >= 0; tamHeap--) {
            reconstruir(subarray, tamHeap, n);
        }
    
        for (int i = n - 1; i >= 0; i--) {
            Personagem temp = subarray.get(0);
            subarray.set(0, subarray.get(i));
            subarray.set(i, temp);
            mov += 3;
    
            reconstruir(subarray, 0, i);
        }
    }
    
    

    public static void reconstruir(List<Personagem> subarray, int i, int tamHeap) {
        int filhoMax = i;
        int esq = 2 * i + 1;
        int dir = 2 * i + 2;
    
        if (esq < tamHeap && (subarray.get(esq).getHairColour().compareTo(subarray.get(filhoMax).getHairColour()) > 0 ||
                (subarray.get(esq).getHairColour().compareTo(subarray.get(filhoMax).getHairColour()) == 0 &&
                subarray.get(esq).getName().compareTo(subarray.get(filhoMax).getName()) > 0))) {
            filhoMax = esq;
        }
        comp++;
        if (dir < tamHeap && (subarray.get(dir).getHairColour().compareTo(subarray.get(filhoMax).getHairColour()) > 0 ||
                (subarray.get(dir).getHairColour().compareTo(subarray.get(filhoMax).getHairColour()) == 0 &&
                 subarray.get(dir).getName().compareTo(subarray.get(filhoMax).getName()) > 0))) {
            filhoMax = dir;
        }
        comp++;
        ...
        comp++;
        if (filhoMax != i) {
            Personagem temp = subarray.get(i);
            subarray.set(i, subarray.get(filhoMax));
            subarray.set(filhoMax, temp);
            reconstruir(subarray, filhoMax, tamHeap);
        }
    }
    ...
    public static int getMaiorFilho (List<Personagem> subarray, int i, int tamHeap) {
        int filho;
        comp++;
        if (2 * i == tamHeap || subarray.get(2*i).getHairColour().compareTo(subarray.get(2*i+1).getHairColour()) > 0 || 
        (subarray.get(2*i).getHairColour().compareTo(subarray.get(2*i+1).getHairColour()) == 0 &&
        subarray.get(2*i).getName().compareTo(subarray.get(2*i+1).getName()) > 0)) {
            filho = 2 * i;
        } else {
            filho = 2 * i + 1;
        }
