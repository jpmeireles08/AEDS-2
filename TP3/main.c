...
int main() {
  ...
  for (int i = 0; i < modificacoes; i++) {
    ...
    while (token != NULL) {
      strcpy(split[splitIndex++], token);
      token = strtok(NULL, " ");
    }

    if (strcmp(split[0], "II") == 0) {
      for (int j = 0; j < n; j++) {
        Personagem *p = (Personagem *)malloc(sizeof(Personagem));
        if (encontrarPersonagemPorID(arquivo, split[1], p)) {
          inserirInicio(p);
          break;
        } else {
          free(p);
    ...
    } else if (strcmp(split[0], "I*") == 0) {
      for (int j = 0; j < n; j++) {
        Personagem *p = (Personagem *)malloc(sizeof(Personagem));
        if (encontrarPersonagemPorID(arquivo, split[2], p)) {
          inserir(p, atoi(split[1]));
          break;
        } else {
          free(p);
        }
      }
    } else if (strcmp(split[0], "RI") == 0) {
      removerInicio();
    } else if (strcmp(split[0], "RF") == 0) {
      removerFim();
    } else if (strcmp(split[0], "R*") == 0) {
      remover(atoi(split[1]));
    }
  }
